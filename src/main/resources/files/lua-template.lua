...
ENT.JumpSound = {
Sound("npc_{{NEXTBOT-NAME}}/{{JUMP-SOUND-FILE}}"), {{REMOVE-IF-NO-JUMP}}
}
ENT.JumpHighSound = {
Sound("npc_{{NEXTBOT-NAME}}/{{JUMP-SOUND-FILE}}"), {{REMOVE-IF-NO-JUMP}}
}
ENT.TauntSounds = {
	Sound("npc_{{NEXTBOT-NAME}}/{{KILL-SOUND-FILE}}"), {{REMOVE-IF-NO-KILL}}
}
local chaseMusic = Sound("npc_{{NEXTBOT-NAME}}/{{CHASE-SOUND-FILE}}") {{REMOVE-IF-NO-CHASE}}
local chaseMusic = nil {{ADD-IF-NO-CHASE}}
...
local npc_{{NEXTBOT-NAME}}_acquire_distance =
	CreateConVar("npc_{{NEXTBOT-NAME}}_acquire_distance", 2500, FCVAR_NONE,
	"The maximum distance at which {{NEXTBOT-NAME}} will chase a target.")

local npc_{{NEXTBOT-NAME}}_spawn_protect =
	CreateConVar("npc_{{NEXTBOT-NAME}}_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, {{NEXTBOT-NAME}} will not target players or hide within 200 units of \z
	a spawn point.")

local npc_{{NEXTBOT-NAME}}_attack_distance =
	CreateConVar("npc_{{NEXTBOT-NAME}}_attack_distance", 80, FCVAR_NONE,
	"The reach of {{NEXTBOT-NAME}}'s attack.")

local npc_{{NEXTBOT-NAME}}_attack_interval =
	CreateConVar("npc_{{NEXTBOT-NAME}}_attack_interval", 0.2, FCVAR_NONE,
	"The delay between {{NEXTBOT-NAME}}'s attacks.")

local npc_{{NEXTBOT-NAME}}_attack_force =
	CreateConVar("npc_{{NEXTBOT-NAME}}_attack_force", 800, FCVAR_NONE,
	"The physical force of {{NEXTBOT-NAME}}'s attack. Higher values throw things \z
	farther.")

local npc_{{NEXTBOT-NAME}}_smash_props =
	CreateConVar("npc_{{NEXTBOT-NAME}}_smash_props", 1, FCVAR_NONE,
	"If set to 1, {{NEXTBOT-NAME}} will punch through any props placed in their way.")

local npc_{{NEXTBOT-NAME}}_allow_jump =
	CreateConVar("npc_{{NEXTBOT-NAME}}_allow_jump", 1, FCVAR_NONE,
	"If set to 1, {{NEXTBOT-NAME}} will be able to jump.")

local npc_{{NEXTBOT-NAME}}_hiding_scan_interval =
	CreateConVar("npc_{{NEXTBOT-NAME}}_hiding_scan_interval", 3, FCVAR_NONE,
	"{{NEXTBOT-NAME}} will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant {{NEXTBOT-NAME}}s are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_{{NEXTBOT-NAME}}_hiding_repath_interval =
	CreateConVar("npc_{{NEXTBOT-NAME}}_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to {{NEXTBOT-NAME}}'s hiding spot will be redetermined every X seconds.")

local npc_{{NEXTBOT-NAME}}_chase_repath_interval =
	CreateConVar("npc_{{NEXTBOT-NAME}}_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of {{NEXTBOT-NAME}}'s target will be redetermined every \z
	X seconds.")

local npc_{{NEXTBOT-NAME}}_expensive_scan_interval =
	CreateConVar("npc_{{NEXTBOT-NAME}}_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_{{NEXTBOT-NAME}}_force_download =
	CreateConVar("npc_{{NEXTBOT-NAME}}_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download {{NEXTBOT-NAME}} resources \z
...
if npc_{{NEXTBOT-NAME}}_force_download:GetBool() then
	resource.AddWorkshop(workshopID)
end

util.AddNetworkString("{{NEXTBOT-NAME}}_nag")
util.AddNetworkString("{{NEXTBOT-NAME}}_navgen")
util.AddNetworkString("{{NEXTBOT-NAME}}_navgen")
...
local VECTOR_{{NEXTBOT-NAME}}_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_{{NEXTBOT-NAME}}_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_{{NEXTBOT-NAME}}: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other {{NEXTBOT-NAME}}s, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "npc_{{NEXTBOT-NAME}}"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "{{NEXTBOT-NAME}}MissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_{{NEXTBOT-NAME}}" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why {{NEXTBOT-NAME}} isn't working.
	net.Start("{{NEXTBOT-NAME}}_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_{{NEXTBOT-NAME}}: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_{{NEXTBOT-NAME}}: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_{{NEXTBOT-NAME}}: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_{{NEXTBOT-NAME}}: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_{{NEXTBOT-NAME}}: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, {{NEXTBOT-NAME}} in pairs(ents.FindByClass("npc_{{NEXTBOT-NAME}}")) do
		{{NEXTBOT-NAME}}:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "{{NEXTBOT-NAME}}NavGen", navEndGenerate)
	else
		print("npc_{{NEXTBOT-NAME}}: nav_generate failed to initialize")
...
concommand.Add("npc_{{NEXTBOT-NAME}}_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_{{NEXTBOT-NAME}}: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. {{NEXTBOT-NAME}} doesn't use
	...
	elseif isConsole then
		print("npc_{{NEXTBOT-NAME}}: Beginning nav_generate requested by server console")
	else
		return
	end
	...
	net.Start("{{NEXTBOT-NAME}}_navgen")
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_{{NEXTBOT-NAME}}_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_{{NEXTBOT-NAME}}_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_{{NEXTBOT-NAME}}_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_{{NEXTBOT-NAME}}_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. {{NEXTBOT-NAME}} means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_{{NEXTBOT-NAME}}_smash_props:GetBool() then continue end
...
function ENT:At{{NEXTBOT-NAME}}tJumpAtTarget()
	-- No double-jumping.
	...
	local maxAttackDistance = npc_{{NEXTBOT-NAME}}_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_{{NEXTBOT-NAME}}_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_{{NEXTBOT-NAME}}_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_{{NEXTBOT-NAME}}_attack_distance:GetInt()
		...
		local repathInterval = npc_{{NEXTBOT-NAME}}_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_{{NEXTBOT-NAME}}_allow_jump:GetBool()
			and currentTime - self.LastJumpScan >= scanInterval
		then
			self:At{{NEXTBOT-NAME}}tJumpAtTarget()
			self.LastJumpScan = currentTime
		end
	else
		local hidingScanInterval = npc_{{NEXTBOT-NAME}}_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_{{NEXTBOT-NAME}}_hiding_repath_interval:GetFloat()
...
local MAT_{{NEXTBOT-NAME}} = Material("npc_{{NEXTBOT-NAME}}/{{NEXTBOT-NAME}}")
killicon.Add("npc_{{NEXTBOT-NAME}}", "npc_{{NEXTBOT-NAME}}/killicon", color_white)
language.Add("npc_{{NEXTBOT-NAME}}", "{{NEXTBOT-NAME}}")
...
local function DevPrint(devLevel, msg)
	if developer:GetInt() >= devLevel then
		print("npc_{{NEXTBOT-NAME}}: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a {{NEXTBOT-NAME}}.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_{{NEXTBOT-NAME}}_music_volume =
	CreateConVar("npc_{{NEXTBOT-NAME}}_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by {{NEXTBOT-NAME}}. (0-1, where 0 is muted)")

-- If another {{NEXTBOT-NAME}} comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, {{NEXTBOT-NAME}}s do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_{{NEXTBOT-NAME}}_PANIC_COUNT {{NEXTBOT-NAME}}s are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of {{NEXTBOT-NAME}}.
local MUSIC_{{NEXTBOT-NAME}}_PANIC_COUNT = 8

local MUSIC_{{NEXTBOT-NAME}}_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_{{NEXTBOT-NAME}}_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("npc_{{NEXTBOT-NAME}}") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("{{NEXTBOT-NAME}}PanicMusicUpdate")
	...
	if panicMusic == nil then
		if IsValid(LocalPlayer()) and chaseMusic ~= nil then
	...
	local userVolume = math.Clamp(npc_{{NEXTBOT-NAME}}_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_{{NEXTBOT-NAME}}" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_{{NEXTBOT-NAME}}_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear {{NEXTBOT-NAME}} taunt us.
...
local function startTimer()
	if not timer.Exists("{{NEXTBOT-NAME}}PanicMusicUpdate") then
		timer.Create("{{NEXTBOT-NAME}}PanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_{{NEXTBOT-NAME}})

	-- Get the normal vector from {{NEXTBOT-NAME}} to the player's eyes, and then compute
	...
	-- {{NEXTBOT-NAME}} should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("{{NEXTBOT-NAME}}HUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("{{NEXTBOT-NAME}}HUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("{{NEXTBOT-NAME}} is studying this map.", "{{NEXTBOT-NAME}}HUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "{{NEXTBOT-NAME}}HUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "{{NEXTBOT-NAME}}HUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "{{NEXTBOT-NAME}}HUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "{{NEXTBOT-NAME}}HUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("{{NEXTBOT-NAME}}_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "{{NEXTBOT-NAME}}NavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. {{NEXTBOT-NAME}} doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("npc_{{NEXTBOT-NAME}}_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for {{NEXTBOT-NAME}} to figure \z
		this map out.\n\z
...
net.Receive("{{NEXTBOT-NAME}}_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! {{NEXTBOT-NAME}} doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet {{NEXTBOT-NAME}}-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! {{NEXTBOT-NAME}} doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to {{NEXTBOT-NAME}}.\n\z
			\n\z
			If you ARE the server host, you can run npc_{{NEXTBOT-NAME}}_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not {{NEXTBOT-NAME}}-compatible!",
			"Ok", nil,
